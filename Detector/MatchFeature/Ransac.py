...
def Ransac(match_kp1, match_kp2):
    ...
    inliers_thresold = 2.5  # Distance threshold for determining variables with equality check
    # If the distance from the projection of the first key point to the second key point is less than the threshold, it fits the homography model.
    # A new match dataset is created for the inliers required to plot the matches
    good_matches = []
    for i, m in enumerate(match_kp1):
        ...
        #THE DISTANCE BETWEEN HOMOGRAPHY AND THE POINT IS CALCULATED
    ...
    print('# Esles me:                            \t', len(match_kp1))
    print('# Inliers i.e. matches matching the given homography:                           \t', len(inliers1))
